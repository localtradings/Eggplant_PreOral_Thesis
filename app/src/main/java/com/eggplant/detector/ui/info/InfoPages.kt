package com.eggplant.detector.ui.info
...
@Composable
fun ScanTipsPage(onBack: () -> Unit) = InfoPage(
    ...
    sections = listOf(
        ...
        InfoSection(localized("Live detection", "Live na pagtukoy"), localized("The packaged YOLO26m model runs locally with NCNN. Wait for a stable box before opening or saving a result.", "Lokal na tumatakbo sa NCNN ang naka-package na YOLO26m model. Hintaying maging stable ang kahon bago buksan o i-save ang resulta.")),
    ),
    onBack = onBack,
)

@Composable
fun PrivacyPage(onBack: () -> Unit) = InfoPage(
    ...
    sections = listOf(
        ...
        InfoSection(localized("Camera permission", "Pahintulot sa kamera"), localized("Camera access is used only for live detection and capture. Gallery selection uses Android's system photo picker.", "Ginagamit lamang ang camera access para sa live detection at capture. Android system photo picker ang gamit sa Gallery.")),
    ),
    onBack = onBack,
)

@Composable
fun HelpPage(onBack: () -> Unit) = InfoPage(
    ...
    sections = listOf(
        ...
        InfoSection(localized("Why do scan tips matter?", "Bakit mahalaga ang payo sa scan?"), localized("Lighting, focus, distance, and motion can change detector confidence and box stability.", "Maaaring baguhin ng ilaw, focus, layo, at paggalaw ang kumpiyansa at stability ng kahon.")),
        InfoSection(localized("How do I change language or units?", "Paano palitan ang wika o yunit?"), localized("Open Settings and choose Language, Theme, or Units. These selections are saved locally.", "Buksan ang Mga Setting at piliin ang Wika, Tema, o Mga Yunit. Lokal itong nase-save.")),
    ),
    onBack = onBack,
)

@Composable
fun AboutPage(onBack: () -> Unit) = InfoPage(
...
@Composable
fun OfflineStatusPage(onBack: () -> Unit) = InfoPage(
    ...
    sections = listOf(
        InfoSection(localized("What works offline", "Gumagana offline"), localized("Live CameraX detection, Gallery analysis, disease library, local history, notifications, settings, and support pages.", "Live CameraX detection, Gallery analysis, aklatan ng sakit, lokal na kasaysayan, abiso, setting, at mga pahina ng tulong.")),
